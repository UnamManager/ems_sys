...
def get_user_list():
    try:
        ...
        # get_all_values()를 써야 '0952' 같은 숫자가 깨지지 않음
        return user_ws.get_all_values() 
    except:
        return []
...
if not st.session_state.logged_in:
    ...
    st.info("본 시스템은 승인된 협력 중개업소만 이용 가능합니다.")
    ...
    with st.form("login_form"):
        ...
        if st.form_submit_button("로그인"):
            found = False
            for row in user_data[1:]: # 첫 줄 ID/PW 헤더 제외
                if len(row) >= 2:
...
with st.sidebar:
    ...
    with st.expander("🛠️ 시스템 설정"):
        admin_pwd = st.text_input("관리자 코드 입력", type="password")
        if admin_pwd == ADMIN_PASSWORD_RES:
            st.session_state.auth_res = True
            st.rerun()
        if admin_pwd == ADMIN_PASSWORD_MANAGE:
