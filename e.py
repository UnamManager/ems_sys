    ...
    try:
        ...
        for i, row in enumerate(data):
            if row[0] == user_id:
                ...
                ws.update(f'C{i+1}', [[datetime.now().strftime("%Y-%m-%d %H:%M:%S")]]); return True
        return True
    except: return True
        ...
        if st.form_submit_button("로그인"):
            if u_id in user_dict and user_dict[u_id] == u_pw:
                ...
                if current_db_key != "" and current_db_key != st.session_state.session_key:
                    st.error("🔒 다른 기기에서 로그인하여 EMS시스템 사용이 제한됩니다."); st.session_state.pending_user = u_id
                else:
...
if choice == "📊 실시간 현황":
    ...
    with tab1:
        ...
        try:
            ...
            else: daily_res = pd.DataFrame()
        except: st.error(f"데이터 로드 오류 ({target_sheet_name})"); st.stop()
        ...
        with st.form("reserve_form"):
            ...
            with col_tel:
                with st.container(border=True):
                    tel_num = "062-511-9336"; st.caption("⚠️취소/변경해야 하는 경우 미리 사전에 연락 부탁드립니다."); st.write(f"**입주촉진센터: {tel_num}**")
            ...
            if submit_btn:
                if not r_name or not r_agency: st.error("성함과 업소명을 모두 입력해주세요.")
                    ...
                    # 저장 시에는 기존 컬럼 구조 유지를 위해 동, 호수 각각에도 정보를 넣되 상세히 기록
                    ...
                    unit_info_str = "".join([f"- {idx+1}세대: {item['동']}동 {item['호수']}호 ({item['타입']})\n" for idx, item in enumerate(r_items)])
                    send_email_notification(f"📢 [EMS] {target_sheet_name} 예약\n■ 일시: {r_date_val} ({t_val})\n■ 예약자: {r_name}({r_agency})\n{unit_info_str}")
    ...
    with tab2:
        ...
        try:
            ...
            df_n = pd.DataFrame(d_n[1:], columns=d_n[0]) if len(d_n) > 1 else pd.DataFrame(columns=d_n[0])
            df_y = pd.DataFrame(d_y[1:], columns=d_y[0]) if len(d_y) > 1 else pd.DataFrame(columns=d_y[0])
            v_all = pd.concat([df_n, df_y], ignore_index=True)
            
            if not v_all.empty:
                if "예약날짜" in v_all.columns: v_all.rename(columns={"예약날짜":"날짜"}, inplace=True)
                if "예약시간" in v_all.columns: v_all.rename(columns={"예약시간":"시간"}, inplace=True)
                if "관람세대수" in v_all.columns: v_all.rename(columns={"관람세대수":"세대수"}, inplace=True)
                v_daily = v_all[v_all["날짜"] == view_date.strftime("%Y-%m-%d")].copy()
                
                # [수정2] 시간순 정렬 (time_slots 리스트 순서 기준)
                v_daily['시간_index'] = v_daily['시간'].apply(lambda x: time_slots.index(x) if x in time_slots else 99)
                v_daily = v_daily.sort_values('시간_index').drop(columns=['시간_index'])
            ...
            if not v_daily.empty:
                ...
                # '동' 컬럼에 이미 매칭된 정보가 들어있으므로 동 컬럼만 보여줘도 충분합니다.
                st.dataframe(v_daily[["예약자", "세대수", "동", "시간"]].rename(columns={"동":"관람세대 정보"}), use_container_width=True, hide_index=True)
            else: st.info("해당 날짜에 등록된 예약이 없습니다.")
        except Exception as e: st.error(f"현황판 로드 오류: {e}")

# 관리자 모드는 기존과 동일하여 유지 (코드량 관계상 하단 생략 가능하나 로직은 보존됨)
