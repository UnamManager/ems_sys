...
def sync_session(user_id, my_key):
    return True # API 부하 방지를 위해 상시 통과
...
# 🎨 스타일 적용 함수 (신규 매물 강조 추가)
def apply_style(df):
    def row_style(row):
        styles = [''] * len(row)
        # 1. '신규' 키워드가 비고란에 있으면 행 전체 노란색 강조
                ...
                try:
                    ...
                    for i, r in enumerate(all_status):
                        if len(r) > 0 and r[0] == u_id:
                            target_row = i + 1; current_db_key = r[1].strip() if len(r) > 1 else ""; break
                    if current_db_key != "" and current_db_key != st.session_state.session_key:
                        ...
                        if target_row != -1: ws_status.update(f'B{target_row}:C{target_row}', [[st.session_state.session_key, now_str]])
                        else: ws_status.append_row([u_id, st.session_state.session_key, now_str])
                        st.session_state.logged_in = True; st.session_state.user_id = u_id; st.rerun()
                except: st.session_state.logged_in = True; st.session_state.user_id = u_id; st.rerun()
            else: st.error("❌ 로그인 정보를 확인해주세요.")
    if "pending_user" in st.session_state:
        if st.button(f"🔑 '{st.session_state.pending_user}' 님의 접속 종료 후 시작"):
            try:
                ws_status = sheet.worksheet("접속현황"); all_status = ws_status.get_all_values()
                for i, r in enumerate(all_status):
...
# =========================
    ...
    with tab1:
        ...
        try:
            ...
            if current_res_count >= 3: st.error(f"🚫 예약 마감 ({current_res_count}/3)"); can_reserve = False
            else: st.info(f"✅ 예약 가능 ({current_res_count}/3)"); st.progress(current_res_count / 3); can_reserve = True
        except: st.warning("⚠️ 시트 연결 중..."); can_reserve = False
        ...
        with st.form("reserve_form"):
            c1, c2 = st.columns(2); r_name = c1.text_input("성함[실명]"); r_agency = c2.text_input("중개업소 명칭")
            memo_input = st.text_area("비고 (특이사항)"); submit_btn = st.form_submit_button("📅 예약 최종 확정", disabled=not can_reserve, use_container_width=True)
            if submit_btn:
                if not r_name or not r_agency: st.error("성함과 업소명을 입력해주세요.")
                else:
                    try:
                        latest_res = target_ws.get_all_values()
                        latest_df = pd.DataFrame(latest_res[1:], columns=latest_res[0]) if len(latest_res) > 1 else pd.DataFrame(columns=COL_NAMES)
                        dup_mask = (latest_df["예약날짜"] == r_date_val.strftime("%Y-%m-%d")) & (latest_df["예약시간"] == t_val) & (latest_df["중개업소"] == r_agency)
                        if not latest_df[dup_mask].empty: st.error("🚫 이미 이 시간대에 예약이 존재합니다."); st.stop()
                        combined_info = " / ".join([f"{it['동']}동 {it['호수']}호" for it in r_items])
                        types_str = ", ".join([s["타입"] for s in r_items]); final_memo = f"[{st.session_state.user_id}] {memo_input}"
                    ...
                    except: st.error("⚠️ 서버 통신 오류. 다시 시도해주세요.")
...
# =========================
    ...
    st.title("⚙️ 매물 통합 관리")
    col1, col2, col3 = st.columns(3); edit_dj = col1.selectbox("수정 단지", ["1단지", "2단지", "3단지"])
    edit_dong = col2.text_input("동 입력"); edit_ho = col3.text_input("호수 입력")
    if edit_dong and edit_ho:
        ...
        if not target_df.empty:
            ...
            with st.form("edit_form"):
                ...
                new_type = st.text_input("타입", value=curr["타입"]); new_status = st.selectbox("거래상태", ["관람가능", "거래완료"], index=0 if curr["거래여부"] == "관람가능" else 1)
                new_price_str = st.text_input("매매가/보증금", value=str(int(curr["매매가_num"]))); new_monthly_str = st.text_input("월세", value=str(int(curr["월세_num"]))); new_note = st.text_input("비고", value=curr["비고"])
                if st.form_submit_button("💾 저장"):
                    try:
                        f_price = f"{int(new_price_str.replace(',','')):,}"; f_monthly = f"{int(new_monthly_str.replace(',','')):,}"; new_sheet_name = f"{edit_dj}_{'매매' if new_gubun == '매매' else '임대'}"
                        ...
                        if idx != -1:
                            ...
                            if old_sheet_name != new_sheet_name: sheet.worksheet(new_sheet_name).append_row(new_data); ws.delete_rows(idx)
