...
# =========================
    ...
    with tab2:
        ...
        try:
            ...
            # 2. 야간 예약 로드 후 해당 단지만 필터링 (수정된 로직)
            ws_y = sheet.worksheet("야간_관람예약"); d_y = ws_y.get_all_values()
            df_y_raw = pd.DataFrame(d_y[1:], columns=d_y[0]) if len(d_y) > 1 else pd.DataFrame(columns=COL_NAMES)
            
            # 야간 데이터 중 '동호수' 컬럼에 선택한 단지(예: '1단지') 텍스트가 포함된 것만 추출
            df_y = df_y_raw[df_y_raw["동호수"].str.contains(sel_dj_view, na=False)].copy()
